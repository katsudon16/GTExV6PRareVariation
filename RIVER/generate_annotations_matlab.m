...
fid = fopen(sprintf('%s/reference/gencode.v19.genes.autosome.lincRNA.gtf',dirNames.RAREVARDIR),'r');
data_anno = textscan(fid,'%d%d%d%s');
fclose(fid);
temp_anno.chr = double(data_anno{1});
temp_anno.pos = [double(data_anno{2}) double(data_anno{3})];
temp_anno.gene_ids = data_anno{4};
...
target_ensembl_ids = intersect(ensembl_ids,temp_anno.gene_ids);
...
for i = 1:length(target_ensembl_ids)
    idx_target(i) = strmatch(target_ensembl_ids(i),temp_anno.gene_ids,'exact');
end
