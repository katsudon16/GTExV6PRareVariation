...
load(sprintf('%s/reference/dataGTEx.mat',dirNames.RAREVARDIR));
load(sprintf('%s/reference/annoGTEx.mat',dirNames.RAREVARDIR));
...
for tis = 1:length(list_tissues)   
    ...
    temp_exp = importdata(sprintf('%s/preprocessing/PEER/dat/%s.peer.ztrans.txt',dirNames.RAREVARDIR,list_tissues{tis}));
    ...
    for i = 1:length(data.indNames.wgs)
        match = strmatch(data.indNames.wgs(i),temp_exp.textdata(1,2:end),'exact');
        if isempty(match) == 0,
            idx_wgs(:,i) = [i match];
        end
        %idx_wgs(:,i) = [i; strmatch(data.indNames.wgs(i),temp_exp.textdata(1,2:end),'exact')];
    end
    idx_wgs(:,sum(isnan(idx_wgs)) > 0) = []
